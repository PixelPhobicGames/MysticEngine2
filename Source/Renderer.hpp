...
void RenderLevel(){
    ...
    for (int x = 0; x <= WorldData.Width - 1;  x ++ ){
        for (int y = 0; y <= WorldData.Height - 1;  y ++ ){
            ...
            { 
                case L'1':
                    DrawTextureEx(WorldData.Tile1, {((x*20) - MysticEngineCamera.X)* MysticEngine.WindowScale,((y*20) - MysticEngineCamera.Y)* MysticEngine.WindowScale}, 0, MysticEngine.WindowScale, WHITE);
                    break;
                case L'2':
